...
class Comments extends React.Component {
	...
	onSaveComment(id) {
		if (this.state.author && this.state.body) {
			...
			this.props.editCurrentComment(id, commentEdit).then(() => this.setState({
				success: true,
				invalid: false
			}))
			this.closeModal();
	...
	render() {
		...
		return (<div className="col-12">
			...
			<ul className="list-group">
				{
					comments.map((comment) => <li className="row pt-3 border-bottom border-gray mx-3" key={comment.id}>
						<div className="row comment_body">
							<div className="col-12 comment-text">
								<p className="pb-3 mb-0 lh-125">
									{comment.body}
								</p>
							</div>
						</div>
						<div className="row comment_detail">
							<div className="col-4" id="title_item_left">
								...
								<time>{moment(comment.timestamp).format("MMM-DD-YYYY hh:mma")}</time>|
						...
						<div className="modal-body" id="post-form">
							<form className="clearfix">
								<div className="form-group">
									<label>author:</label>
                                    <input type="text" className="form-control" id="title" value={this.state.author} onChange={(e) => this.onChangeAuthor(e)}/>

								</div>

								<div className="form-group">
									<label>Content:</label>
                                    <textarea className="form-control" id="content" rows="3" value={this.state.body} onChange={(e) => this.onChangeComment(e)}>
									</textarea>
								</div>
							</form>
						</div>
